...
log_path = 'good_request.log'
output_csv_log = '2good_req.csv'
class_flag = "good"
...
class LogParse:
    ...
    def extract_features(method, path_enc, body_enc, headers):
        # Count percentages and spaces in the raw (encoded) URL and body
        ...
        if raw_percentages > 3 and raw_spaces > 3:
        ...
        special_chars = sum(path.count(c) + body.count(c) for c in '$&|')
        ...
        return [method, path_enc, body_enc, single_q, double_q, dashes, braces, spaces, raw_percentages_count,
                semicolons, angle_brackets, special_chars, path_length, body_length, badwords_count, class_flag]
...
def parse_log(log_path):
    ...
    try:
        ...
        print(
            '[+] Oops..! Please make sure binary data is not present in Log, like raw image dump, flash (.swf files) dump etc.')
        exit()
